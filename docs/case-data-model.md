...
> **本体层说明**：本模型描述的是**认知层**（investigation-ontology skills）的数据结构。在此基础上，每个案件还需要同步维护**本体层**的实体和关系文件（`global_ontology/entities/`、`global_ontology/relations/`），通过 `ontology_ref` 字段与认知层节点关联。详见 `docs/ontology/design-overview.md`。
...
| **本体层绑定** | ENT 和 EV 节点必须包含 `ontology_ref` 字段指向 `global_ontology/entities/` 中的本体对象，详见 `/ontology` skill → `references/binding-protocol.md` |

**节点类型**：

| 前缀 | 类型 | 文件格式 | 说明 | 本体层映射 |
|------|------|----------|------|-----------|
| EV- | evidence | JSON/MD | 原始证据注册和详细分析 | 必须含 `ontology_ref` → `global_ontology/entities/evidence/` |
| LS- | clue | MD | 从原始证据中提炼的线索 | 不映射 |
| ARG- | argument | MD | 基于线索的论据构建 | 不映射 |
| FND- | finding | MD | 基于论据的事实认定 | 引用 `global_ontology/relations/` (仅 HARD) |
| ENT- | entity | JSON | 涉案实体详情 | 必须含 `ontology_ref` → `global_ontology/entities/{type}/` |
