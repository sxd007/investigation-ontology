...
AI 是**辅助性工具**——帮助探索（多角度分析）、帮助分析（结构化框架）、帮助批判（竞争假设推演与替代解释复核），但**不替代调查员的独立判断**。最终调查结论的责任主体始终是调查员。
...
- 初始假设**不得全部为正向**（"举报为真"类）
- **必须至少包含一个真正挑战核心命题的合理替代假设**（如行为未发生、异常来自正常业务或数据质量、责任主体或范围不同、现有材料不足以证明主观故意）
- 替代假设与指控性假设**同等优先级**验证；不得仅以“举报人动机不纯”代替对举报内容真实性的验证
- 形成最小充分的竞争假设集，通常 3-5 个；行为/动机/机会/关系/影响五类用于覆盖检查，不是数量配额
...
| 维度 | 要求 |
|------|------|
| 信息源标注 | 所有 claim 标注来源类型（firsthand/relay/opinion/attitude/inference）和核实来源；详细规则见 `rules/evidence-rules.md` |
| [举报人称] | 举报人提供的信息必须标注 `[举报人称]`，不得直接引用为已确立事实 |
| 置信度 | HYP 使用可选 `0..1` 分析值；EV/FND 使用 suspected / probable / confirmed；not_applicable 属于适用性或处置状态 |
| 双向索引 | 新增信息必须指回原定义文件 |
