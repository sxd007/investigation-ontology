...
读取 `manifests/install-profiles.json`（相对**插件根**；源码树位于 `plugins/investigation-ontology/manifests/`，安装副本位于插件根 `manifests/`），向用户展示可选角色画像及其技能集，引导选择其一。画像是 install-profiles.json 在运行期的消费方——决定本团队默认聚焦哪些技能。
...
3. 按 H2 节逐节对比:
   ...
   - 配置中已填写的字段 → 保留不动
3.5 画像模块值漂移检测（升级场景必查）:
   - READ <plugin-root>/manifests/install-profiles.json
   - 取配置中 Selected Profile 对应的 modules 列表
   - 与配置"角色画像"节的 Active Skills 做集合比对
   - 差异项（新增 / 移除）→ 标记"画像模块待同步"，展示 diff
     后由用户确认是否更新（不自动覆盖，保护手工调整）
4. 如存在"新配置项"或"待补全"项:
   → "插件升级后新增了以下配置项，需要补充："
   → 逐项引导填写
5. 如存在"画像模块待同步"项:
   → "画像定义已扩容/调整，与配置的 Active Skills 存在差异："
   → 展示差异，用户确认后更新 Active Skills
6. 无任何差异 → "无需变更，配置已是最新"
```
...
> 插件的 SessionStart hook 会读取工作区中的完整 `INVESTIGATION-HANDBOOK.md`，并将全文写入平台对应上下文文件的
> `<!-- efio:handbook-start -->` 至 `<!-- efio:handbook-end -->` 标记块。hook 具有双重自愈机制：
> 1. INVESTIGATION-HANDBOOK.md 缺失时自动从模板复制
> 2. 平台上下文文件中全文标记块缺失或内容过期时自动重新注入
