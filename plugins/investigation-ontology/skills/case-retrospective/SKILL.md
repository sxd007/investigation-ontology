...
description: 案件回顾与复盘技术 — 完结案件的多维度复盘框架，聚焦调查员的能力、经验、逻辑、工作方法。当用户提到"复盘""回顾总结""做得怎么样"等复盘意图时触发。独立于案件调查流程，目标为提升调查员觉知而非产出案件结论。
origin: efio
---
...
如存在 `case_memory/`，在正式评分前执行**只读检查**：

1. 按 `investigation-memory` 检查或在内存中重建索引视图，但不覆写案件文件
2. 对照证据注册表、假设和阶段产物识别状态或关联不一致，形成“归档修正建议”，不在评分前直接修改
3. 没有 `Schema Version` 的旧条目及旧版完整推理型 `think` 条目继续读取并标注为 legacy，不因新规则而跳过
4. 重复、超过 30 条或疑似低价值的条目仍纳入本次复盘输入，仅在报告中提出结案归档建议

复盘只把 memory 用作“当时如何判断”的过程材料，不把它用作案件事实、证据或结论来源。状态同步、压缩和清理由 `case-manager` 在结案归档时执行，避免复盘前改写历史输入造成后见之明偏差。
...
**评估要点**：
- 是否在 INIT 阶段形成了最小充分的竞争假设集，并包含真正挑战核心命题的合理替代解释
- 是否以同等标准验证指控性假设和合理替代假设
- 是否主动寻找证伪证据而非仅收集证实证据
...
**参考数据源**：
- 案件中的 AI 对话记录（如保存）
- `case_memory/` → think 类条目（路径暂缓、假设修正等关键决策理由）
