...
**不需要手动替换数据。** 证据链可视化由 AI 在调查过程中根据登记的证据数据自动生成。

**两种可视化模式：**

| 模式 | 命令 | 适用场景 | 输出 |
|------|------|---------|------|
| 交互式 HTML | `/evidence visualize` | 完整交互图、汇报展示 | 独立 HTML 文件（四视图） |
| 对话内 Mermaid 预览 | `/evidence graph` | 快速预览推理链结构 | Mermaid 代码块（对话内渲染） |

**操作步骤：**

1. 在调查推进中向 AI 描述案情、登记证据（通过 `/evidence add` 命令或直接对话）
2. 输入 `/evidence visualize`（或告诉 AI：**"生成一份证据链可视化 HTML"**）
3. AI 自动读取你的案件数据，输出一份完整的 HTML 文件到你的案件目录
4. 用浏览器打开 HTML，交互式审视推理链路

> 💡 如果只想快速看一下推理链结构，不需要完整交互图，用 `/evidence graph` 在对话内直接渲染 Mermaid 预览。
...
| 呈现    | 生成可视化 HTML、截屏入报告            | `/evidence visualize` + 浏览器     |
...
→ 或直接 `/evidence integrity` + `/evidence graph` 快速预览
