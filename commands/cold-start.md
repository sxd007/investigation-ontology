...
## Usage
```
/efio:cold-start           完整设置（10-15分钟）
/efio:cold-start --quick   快速设置（仅团队信息）
/efio:cold-start --check-integrations  仅检查 MCP 集成
```

## Process
遵循 cold-start 技能的 5 阶段流程：
...
## 注意
- 配置写入 `{配置路径}/`（持久化，更新不丢失）；路径按平台决定（见 `config-templates/config-loader.md § 平台路径`）
- 所有技能在运行前都会检查配置是否完成
- 编辑 `{配置路径}/team-profile.md` 可随时修改
