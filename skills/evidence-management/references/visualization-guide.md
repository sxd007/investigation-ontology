...
description: 证据链可视化指南 — v3 多视图架构（Reasoning/Hypotheses/Governance/Issues）、7个数据块、边语义分色、治理徽章、两种可视化模式、scan-chain.py 完整选项参考。在生成可视化或使用 scan-chain.py 时加载。
---
...
所有节点文件**只使用** `relations:` 字段声明关系（见 `references/node-graph-spec.md`）。旧版 `sources:` 字段已废弃（`scan-chain.py --integrity` 会发出 WARN）。
...
HTML 模板来源：`templates/evidence-chain-viz/evidence_chain_viewer.html`。`evidence_chain_injector.js` 负责将案件数据注入模板，生成零外部依赖的独立 HTML 文件。

**手动调用 injector（调试场景）：**

```bash
node templates/evidence-chain-viz/evidence_chain_injector.js \
