...
使用 `skills/investigation-foundation/SKILL.md` 的交互式推理协议形成最小充分的竞争假设集，通常 3-5 个。行为、动机、机会、关系、影响五类是覆盖检查面，不是每类必须生成 1-2 个的配额；与当前问题无关的类别可标记不适用并说明理由。
...
初始假设不得全部支持举报或指控，至少包含一个真正挑战核心命题的合理替代解释，例如行为未发生、异常来自正常业务或数据质量、责任主体或范围不同、现有材料不足以证明主观故意。不得仅以“举报人动机不纯”代替对举报内容真实性的验证。

**结构化登记**：生成的假设清单需同时：
   1. 写入 `evidence_registry.json` 的 `hypotheses[]` 数组，每条 hypothesis 填写 `hypothesis_id`（HYP-NNN）、`statement`、`status=active`；有可说明的依据时填写初始 `confidence`（0-1），不得凭语感制造精确概率
...
### 竞争假设（3-5个）
1. {假设A} — 初始判断及依据: {数值或方向性说明}
2. {假设B} — 初始判断及依据: {数值或方向性说明}
3. {合理替代假设} — 初始判断及依据: {数值或方向性说明}
...
- **背景摘要** — 案件来源、关键事实、舞弊类型
- **竞争假设** — 最小充分集合，通常 3-5 个，含合理替代假设、初始判断及依据
